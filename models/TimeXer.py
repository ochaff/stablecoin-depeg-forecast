...
class FlattenHead(nn.Module):
    def __init__(self, n_vars, nf, target_window, method,  head_dropout=0):
        super().__init__()
        self.n_vars = n_vars
        ...
        if method == 'HierE2E':
            self.linearstd = nn.Linear(nf, target_window)
            self.std_activ = nn.Softplus()
        self.method = method
        

    def forward(self, x):  # x: [bs x nvars x d_model x patch_num]
        if self.method == 'HierE2E':
            x = self.flatten(x)
            std = self.linearstd(x)
            x = self.linear(x)
            x = self.dropout(x)
...
class Model(nn.Module):
    ...
    def __init__(self, seq_len, pred_len, patch_len, revin, enc_in, d_model, d_ff, dropout, embed, freq, factor, n_heads, activation, e_layers, method):
        ...
        self.n_vars = enc_in
        self.method = method
        self.revin = RevIN(self.n_vars, affine = True)
        # Embedding
        ...
        self.head = FlattenHead(enc_in, self.head_nf, pred_len,
                                head_dropout= dropout, method = method)
    ...
    def forward(self, x_enc, x_exo, x_mark_enc = None, x_dec = None, x_mark_dec = None, mask=None):   
        dec_out = self.forecast_multi(x_enc, x_exo, x_mark_enc, x_dec, x_mark_dec)
