# ============================================================
# NeuralForecast TimeXer benchmark with full historical exogenous features
# ============================================================
#
# Output:
#   ./benchmark_outputs/timeXer_hist_exog/preds_test_set.pkl
#
# This file has:
#   A["true"]   : (B,H)
#   A["Q"]      : (B,H,J)
...
out_dir = Path("./benchmark_outputs/timeXer_hist_exog")
out_dir.mkdir(parents=True, exist_ok=True)

pd.Series(hist_exog_list, name="hist_exog").to_csv(
    out_dir / "timeXer_hist_exog_features.csv",
    index=False,
)
...
def detect_timeXer_point_col(cv_df):
    """
    Detect TimeXer median / point forecast column.
    NeuralForecast usually outputs "TimeXer".
    """
    reserved = {"unique_id", "ds", "cutoff", "y"}

    if "TimeXer" in cv_df.columns:
        return "TimeXer"

    if "TimeXer-median" in cv_df.columns:
        return "TimeXer-median"
    ...
    raise ValueError(
        f"Could not detect TimeXer point column. "
...
def cv_df_to_quantile_pickle_dict(
    ...
    print(
        f"Converted TimeXer hist-exog forecasts: "
...
def fit_save_timeXer_hist_exog(
    Y_df_exog,
    hist_exog_list,
    h,
    n_windows,
    levels,
    u_grid,
    out_dir="./benchmark_outputs/timeXer_hist_exog",
    freq="h",
    input_size=168,
    max_steps=1000,
    val_size=24 * 30,
    ...
    model = TimeXer(
        ...
        # TimeXer's n_series is the number of target series (unique_ids),
        # not the number of exogenous features. We have a single series.
        n_series=1,
    ...
    print("Running NeuralForecast TimeXer with historical exogenous variables...")
    ...
    raw_path = out_dir / "timeXer_hist_exog_cv.parquet"
    cv_df.to_parquet(raw_path, index=False)

    print(f"Saved raw TimeXer CV dataframe: {raw_path.resolve()}")
    print(cv_df.head())
    print(cv_df.columns)

    point_col = detect_timeXer_point_col(cv_df)

    A_timeXer = cv_df_to_quantile_pickle_dict(
        cv_df=cv_df,
        u_grid=u_grid,
        point_col=point_col,
        interval_prefix="TimeXer",
    ...
    save_preds_pickle(A_timeXer, preds_path)

    return cv_df, str(preds_path), A_timeXer


timeXer_cv_df, timeXer_path, A_timeXer = fit_save_timeXer_hist_exog(
    Y_df_exog=Y_df_exog,
    hist_exog_list=hist_exog_list,
    h=h,
    n_windows=n_windows,
...
print("\nSaved TimeXer hist-exog benchmark path:")
print(timeXer_path)